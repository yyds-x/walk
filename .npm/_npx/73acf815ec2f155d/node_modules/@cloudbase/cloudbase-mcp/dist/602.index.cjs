...
class InteractiveServer {
    ...
    setupExpress() {
        ...
        this.app.post("/api/open-url", async (req, res) => {
            ...
            if (!validation.safe) {
                (0, logger_js_1.warn)(`Blocked open-url request for unsafe URL: ${url} (${validation.reason})`);
                ...
                try {
                    ...
                    if (data.type === 'refreshEnvList') {
                        ...
                        try {
                            ...
                            if (targetSessionId) {
                                ...
                                if (sessionData && sessionData.manager) {
                                    ...
                                    try {
                                        ...
                                        (0, logger_js_1.debug)("DescribeEnvs failed, falling back to listEnvs():", error instanceof Error ? error : new Error(String(error)));
                                    ...
                                    (0, logger_js_1.info)(`Environment list refreshed, found ${envs.length} environments`);
                                }
                                else {
                                    ws.send(JSON.stringify({
                                        type: 'envListRefreshed',
                            ...
                            (0, logger_js_1.error)("Failed to refresh environment list", err instanceof Error ? err : new Error(String(err)));
                            ws.send(JSON.stringify({
                                type: 'envListRefreshed',
                                success: false,
    ...
    async start() {
        if (this.isRunning) {
            (0, logger_js_1.debug)(`Interactive server already running on port ${this.port}`);
            return this.port;
        }
        return new Promise((resolve, reject) => {
            (0, logger_js_1.info)("Starting interactive server...");
            ...
            const tryNextPort = () => {
                if (currentIndex >= tryPorts.length) {
                    ...
                    (0, logger_js_1.error)("Server start failed", err);
                    reject(err);
                    return;
                }
                const portToTry = tryPorts[currentIndex];
                currentIndex++;
                (0, logger_js_1.debug)(`Trying to start server on port ${portToTry} (attempt ${currentIndex}/${tryPorts.length})`);
                tryPort(portToTry);
            };
            const tryPort = (portToTry) => {
                ...
                const errorHandler = (err) => {
                    if (err.code === "EADDRINUSE") {
                        (0, logger_js_1.warn)(`Port ${portToTry} is in use, trying next port...`);
                        // 清理当前尝试
                        this.server.removeAllListeners("error");
                        this.server.removeAllListeners("listening");
                        tryNextPort();
                        ...
                        (0, logger_js_1.error)("Server error", err);
                        reject(err);
                    }
                };
                ...
                const listeningHandler = () => {
                    ...
                    if (address && typeof address === "object") {
                        ...
                        (0, logger_js_1.error)("Server start error", err);
                        reject(err);
                    }
                };
    ...
    async stop() {
        if (!this.isRunning) {
            (0, logger_js_1.debug)("Interactive server is not running, nothing to stop");
            return;
        }
        (0, logger_js_1.info)("Stopping interactive server...");
        return new Promise((resolve, reject) => {
            // 设置超时，防止无限等待
            const timeout = setTimeout(() => {
                (0, logger_js_1.warn)("Server close timeout, forcing cleanup");
                this.isRunning = false;
                this.port = 0;
                resolve();
            }, 30000);
            try {
                ...
                this.wss.close(() => {
                    (0, logger_js_1.debug)("WebSocket server closed");
                    // WebSocket关闭后，再关闭HTTP服务器
                    this.server.close((err) => {
                        clearTimeout(timeout);
                        if (err) {
                            (0, logger_js_1.error)("Error closing server:", err);
                            reject(err);
                        }
                        else {
                            (0, logger_js_1.info)("Interactive server stopped successfully");
                            this.isRunning = false;
                            this.port = 0;
                            // 重新创建整个服务器实例以便下次使用
                            this.server = http_1.default.createServer(this.app);
                ...
                clearTimeout(timeout);
                (0, logger_js_1.error)("Error stopping server:", err instanceof Error ? err : new Error(String(err)));
                this.isRunning = false;
                this.port = 0;
                reject(err);
            }
    ...
    async collectEnvId(availableEnvs, accountInfo, errorContext, // EnvSetupContext
    manager, // CloudBase manager instance for refreshing env list
    mcpServer) {
        try {
            // CRITICAL: Clean up any existing unresolved request to prevent hanging
            if (this.currentResolver) {
                (0, logger_js_1.warn)("[collectEnvId] Found existing unresolved request, cleaning up...");
            ...
            (0, logger_js_1.debug)(`[collectEnvId] Received mcpServer parameter: type=${typeof mcpServer}, is null=${mcpServer === null}, is undefined=${mcpServer === undefined}, has server=${!!mcpServer?.server}, has ide=${!!mcpServer?.ide}`);
            (0, logger_js_1.debug)(`[collectEnvId] Current this._mcpServer before update: type=${typeof this._mcpServer}, is null=${this._mcpServer === null}, is undefined=${this._mcpServer === undefined}, has server=${!!this._mcpServer?.server}, has ide=${!!this._mcpServer?.ide}`);
            ...
            if (mcpServer) {
                this._mcpServer = mcpServer;
                (0, logger_js_1.debug)(`[collectEnvId] Updated mcpServer from parameter, has server: ${!!mcpServer?.server}, has ide: ${!!mcpServer?.ide}`);
            }
            else if (this._mcpServer) {
                (0, logger_js_1.debug)(`[collectEnvId] mcpServer parameter is falsy, using existing this._mcpServer`);
            }
            else {
                (0, logger_js_1.warn)(`[collectEnvId] WARNING: Both mcpServer parameter and this._mcpServer are undefined! IDE detection will fail.`);
            }
            (0, logger_js_1.debug)(`[collectEnvId] Effective mcpServer: type=${typeof effectiveMcpServer}, has server=${!!effectiveMcpServer?.server}, has ide=${!!effectiveMcpServer?.ide}`);
            (0, logger_js_1.info)("Starting environment ID collection...");
            (0, logger_js_1.debug)(`Available environments: ${availableEnvs.length}`);
            (0, logger_js_1.debug)(`Account info:`, accountInfo);
            (0, logger_js_1.debug)(`Error context:`, {
            ...
            (0, logger_js_1.debug)(`Created session: ${sessionId}`);
            setTimeout(() => {
                this.sessionData.delete(sessionId);
                (0, logger_js_1.debug)(`Session ${sessionId} expired`);
            }, 5 * 60 * 1000);
            const url = `http://localhost:${port}/env-setup/${sessionId}`;
            (0, logger_js_1.info)(`Opening browser: ${url}`);
            ...
            try {
                ...
                (0, logger_js_1.debug)(`[collectEnvId] Using effectiveMcpServer for openUrl: type=${typeof effectiveMcpServer}, has server=${!!effectiveMcpServer?.server}, has ide=${!!effectiveMcpServer?.ide}`);
                (0, logger_js_1.debug)(`[collectEnvId] effectiveMcpServer keys: ${effectiveMcpServer ? Object.keys(effectiveMcpServer).slice(0, 10).join(', ') : 'null'}`);
                ...
                if (isCodeBuddy && openUrlResult === undefined) {
                    ...
                    (0, logger_js_1.debug)("[collectEnvId] CodeBuddy notification sent, no browser opened");
                }
                (0, logger_js_1.info)("Browser opened successfully");
            }
            catch (browserError) {
                (0, logger_js_1.error)("Failed to open browser", browserError instanceof Error ? browserError : new Error(String(browserError)));
                (0, logger_js_1.warn)(`Please manually open: ${url}`);
            }
            ...
            return new Promise((resolve) => {
                this.currentResolver = (result) => {
                    ...
                    this.currentResolver = null;
                    this.stop().catch((err) => {
                        (0, logger_js_1.debug)("Error stopping server after user selection:", err);
                    });
                    resolve(result);
                };
                const timeoutId = setTimeout(() => {
                    if (this.currentResolver) {
                        (0, logger_js_1.warn)(`Request timeout after ${timeoutDuration / 1000} seconds, resolving with cancelled`);
                        this.currentResolver = null;
                        // 超时后也关闭服务器
                        this.stop().catch((err) => {
            ...
            (0, logger_js_1.error)("Error in collectEnvId", err instanceof Error ? err : new Error(String(err)));
            throw err;
        }
    }
    ...
    getEnvSetupHTML(envs, accountInfo, errorContext, // EnvSetupContext
    sessionId) {
        // Extract error information
        const initTcbError = errorContext?.initTcbError;
        const createEnvError = errorContext?.createEnvError;
        (0, logger_js_1.debug)("getEnvSetupHTML called with:", {
        ...
        return (0, index_js_1.renderEnvSetupPage)({
            envs,
            accountInfo,
            errorContext,
            sessionId,
    ...
    getEnvSetupHTML_OLD(envs, accountInfo, errorContext, sessionId) {
    ...
    <div class="modal">
        ...
        <div class="content">
            ...
            <div class="options">
                ${availableOptions
            .map((option, index) => {
            const isCancel = option.includes("取消") ||
...
exports.InteractiveServer = InteractiveServer;
// 单例实例
let interactiveServerInstance = null;
function getInteractiveServer(mcpServer) {
    if (!interactiveServerInstance) {
        ...
        (0, logger_js_1.debug)(`[getInteractiveServer] Updated mcpServer, has server: ${!!mcpServer?.server}, has ide: ${!!mcpServer?.ide}`);
    }
    return interactiveServerInstance;
}
async function resetInteractiveServer() {
    if (interactiveServerInstance) {
        try {
            await interactiveServerInstance.stop();
        }
        catch (err) {
            (0, logger_js_1.error)("Error stopping existing server instance:", err instanceof Error ? err : new Error(String(err)));
        }
        interactiveServerInstance = null;
    }
}
async function getInteractiveServerSafe(mcpServer) {
    // 如果当前实例存在但不在运行状态，先清理
    if (interactiveServerInstance && !interactiveServerInstance.running) {
        try {
            ...
            (0, logger_js_1.debug)("Error stopping non-running server:", err instanceof Error ? err : new Error(String(err)));
        }
        interactiveServerInstance = null;
    }
