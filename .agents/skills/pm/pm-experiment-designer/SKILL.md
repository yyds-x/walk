...
description: 设计可执行的 A/B 实验方案（假设/分组/指标/样本量/判定与止损）。用户提到 A/B、灰度、样本量、显著性检验等实验关键词时触发。
description_zh: |
  ...
  不适用于：纯数据分析、纯埋点设计、纯 PRD 写作。
description_en: Designs statistically sound A/B experiments with hypotheses, metrics, sample size, decision rules, and guardrails. Invoke when users ask for experiment planning or evaluation.
version: 1.0.0
---
